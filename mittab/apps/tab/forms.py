...
def validate_speaks(value):
    lower_bound = 5   # defaults
    upper_bound = 45
    try:
        lower_bound = int(TabSettings.objects.get(key="lower_speaks").value)
        upper_bound = int(TabSettings.objects.get(key="upper_speaks").value)
    except Exception as e:
        print str(e.message)

    if not (lower_bound <= value <= upper_bound or value == 0):
        raise ValidationError('{0} is outside [{1}, {2}]'.format(value, lower_bound, upper_bound))
