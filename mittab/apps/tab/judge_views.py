...
def add_scratches(request, judge_id, number_scratches):
    ...
    if request.method == 'POST':
        ...
        forms = [
            ScratchForm(
                prefix=str(i), 
                initial={'judge': judge_id, 'scratch_type': 0}) 
            for i in xrange(1, number_scratches + 1)
        ]
    return render_to_response(
...
def view_scratches(request, judge_id):
    ...
    delete_links = [
        "/judge/{}/scratches/delete/{}".format(judge_id, scratches[i].id)
        for i in xrange(number_scratches)]
    links = [
        ('/judge/{}/scratches/add/1/'.format(judge_id), 'Add Scratch', False)]
