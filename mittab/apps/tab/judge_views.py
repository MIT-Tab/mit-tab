...
def view_judges(request):
    ...
    c_judge = [
        (judge.pk, judge.name + ' (' + judge.ballot_code + ') ', flags(judge), TabFlags.flags_to_symbols(flags(judge)))
        for judge in Judge.objects.order_by("name")]

    all_flags = [[TabFlags.JUDGE_CHECKED_IN_CUR, TabFlags.JUDGE_NOT_CHECKED_IN_CUR, TabFlags.JUDGE_CHECKED_IN_NEXT,
...
def add_scratches(request, judge_id, number_scratches):
    ...
    if request.method == 'POST':
        ...
        forms = [ScratchForm(prefix=str(i), initial={'judge': judge_id, 'scratch_type': 0}) for i in
...
def view_scratches(request, judge_id):
    ...
    delete_links = ["/judge/" + str(judge_id) + "/scratches/delete/" + str(scratches[i].id) for i in
                    range(len(scratches))]
