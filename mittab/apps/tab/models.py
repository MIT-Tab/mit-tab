...
class TabSettings(models.Model):
    ...
    def delete(self, using=None, keep_parents=False):
        cache_logic.invalidate_cache(
            "tab_settings_%s" % self.key, cache_logic.PERSISTENT
        )
...
class Debater(models.Model):
    ...
    def save(
        ...
        while (
            not self.tiebreaker
            or Debater.objects.filter(tiebreaker=self.tiebreaker).exists()
        ):
...
class Team(models.Model):
    ...
    hybrid_school = models.ForeignKey(
        "School",
        blank=True,
        null=True,
    ...
    team_code = models.CharField(max_length=255, blank=True, null=True, unique=True)
    ...
    def save(
        ...
        while (
            not self.tiebreaker
            or Team.objects.filter(tiebreaker=self.tiebreaker).exists()
        ):
            self.tiebreaker = random.choice(range(0, 2**16))
...
class BreakingTeam(models.Model):
    VARSITY = 0
    NOVICE = 1
    TYPE_CHOICES = ((VARSITY, "Varsity"), (NOVICE, "Novice"))

    team = models.OneToOneField(
        "Team", on_delete=models.CASCADE, related_name="breaking_team"
    )

    seed = models.IntegerField(default=-1)
    effective_seed = models.IntegerField(default=-1)

    type_of_team = models.IntegerField(default=VARSITY, choices=TYPE_CHOICES)
...
class Judge(models.Model):
    ...
    ballot_code = models.CharField(max_length=255, blank=True, null=True, unique=True)
    ...
    def affiliations_display(self):
        return ", ".join(
            [school.name for school in self.schools.all() if not school.name == ""]
        )
...
class Room(models.Model):
    ...
    def is_checked_in_for_round(self, round_number):
        return RoomCheckIn.objects.filter(room=self, round_number=round_number).exists()
...
class Outround(models.Model):
    ...
    type_of_round = models.IntegerField(default=VARSITY, choices=TYPE_OF_ROUND_CHOICES)
    gov_team = models.ForeignKey(
        Team, related_name="gov_team_outround", on_delete=models.CASCADE
    )
    opp_team = models.ForeignKey(
        Team, related_name="opp_team_outround", on_delete=models.CASCADE
    )
    ...
    CHOICES = ((UNKNOWN, "No"), (GOV, "Gov"), (OPP, "Opp"))
    choice = models.IntegerField(default=UNKNOWN, choices=CHOICES)
    ...
    def __str__(self):
        return "Outround {} between {} and {}".format(
            self.num_teams, self.gov_team, self.opp_team
        )
...
class Round(models.Model):
    ...
    gov_team = models.ForeignKey(
        Team, related_name="gov_team", on_delete=models.CASCADE
    )
    opp_team = models.ForeignKey(
        Team, related_name="opp_team", on_delete=models.CASCADE
    )
    chair = models.ForeignKey(
        Judge, null=True, blank=True, on_delete=models.CASCADE, related_name="chair"
    )
    ...
    def __str__(self):
        return "Round {} between {} and {}".format(
            self.round_number, self.gov_team, self.opp_team
        )
...
class NoShow(models.Model):
    ...
    def __str__(self):
        return (
            str(self.no_show_team) + " was no-show for round " + str(self.round_number)
        )
...
class RoomCheckIn(models.Model):
    ...
    def __str__(self):
        return "Room %s is checked in for round %s" % (self.room, self.round_number)
