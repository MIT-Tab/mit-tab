...
def view_scratches(request, team_id):
    ...
    team = Team.objects.get(pk=team_id)
    if request.method == 'POST':
        forms = [ScratchForm(request.POST,
                             prefix=str(i),
                             instance=scratches[i - 1]) for i in range(1,
                                                                       number_scratches + 1)]
        all_good = True
        for form in forms:
            all_good = all_good and form.is_valid()
    ...
    delete_links = ["/team/" +
                    str(team_id) +
                    "/scratches/delete/" +
                    str(scratches[i].id) for i in range(len(scratches))]
    links = [
        ('/team/' +
         str(team_id) +
         '/scratches/add/1/',
...
def tab_card(request, team_id):
    ...
    for r in rounds:
        dstat1 = [
            k for k in RoundStats.objects.filter(
                debater=d1).filter(
                round=r).all()]
        dstat2 = []
        if (not iron_man):
