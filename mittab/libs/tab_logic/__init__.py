...
def add_scratches_for_single_judge_for_school_affiliation(judge, teams):
    """
    Generates a list of scratch objects to add for a given judge.
    """
    to_return = []
    ...
    for team in teams:
        ...
        if team.school in judge_schools or team.hybrid_school in judge_schools:
            to_return.append(
                Scratch(judge=judge, team=team, scratch_type=Scratch.SCHOOL_SCRATCH)
            )
...
def add_scratches_for_school_affil():
    ...
    Scratch.objects.filter(scratch_type=Scratch.SCHOOL_SCRATCH).all().delete()
    ...
    for judge in all_judges:
        for team in all_teams:
            ...
            if team.school in judge_schools or team.hybrid_school in judge_schools:
                to_create.append(
                    add_scratches_for_single_judge_for_school_affiliation(judge, team)
                )
