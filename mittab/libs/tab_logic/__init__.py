...
def middle_of_bracket_teams():
    ...
    for team in Team.objects.filter(checked_in=True):
        ...
        avg_speaks_rounds += NoShow.objects.filter(
            no_show_team=team, lenient_late=True
        ).count()
...
def perfect_pairing(list_of_teams):
    ...
    for pair in pairings_num:
        if pair < len(list_of_teams):
            if (
                [list_of_teams[pairings_num.index(pair)], list_of_teams[pair]]
                not in all_pairs
                and [list_of_teams[pair], list_of_teams[pairings_num.index(pair)]]
                not in all_pairs
            ):
                all_pairs += [
                    [list_of_teams[pairings_num.index(pair)], list_of_teams[pair]]
