...
def get_tab_card_data(request, team_id):
    ...
    team = Team.objects.prefetch_related("gov_team", "opp_team", "debaters").get(pk=team_id)
    rounds = (
    ...
    for i in range(cur_round - 1):
        if round_stats[i][6] == blank and blank not in round_stats[i + 1][:5]: #Don't fill in totals for incomplete rounds
